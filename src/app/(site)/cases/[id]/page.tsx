    ...
    <div className="pt-32 pb-20">
      <div className="container">
        <nav className="text-sm text-white/40 mb-8">
          ...
          <Link href="/cases" className="hover:text-white">案例展示</Link>
          <span className="mx-2">/</span>
          ...
          <div className="space-y-6">
            <div>
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                {item.featured && (
                  <span className="px-2 py-0.5 rounded-full bg-gold-500/20 text-gold-400 text-xs">⭐ 精选</span>
                )}
              </div>
              <h1 className="heading-3 mb-3">{item.title}</h1>
              {(item.carBrand || item.carModel) && (
                <div className="text-white/60">
                  车型：<span className="text-white">{[item.carBrand, item.carModel].filter(Boolean).join(' ')}</span>
                </div>
              )}
              {item.completedAt && (
            ...
            {item.description && (
              <div>
                ...
                <div className="text-white/70 leading-relaxed whitespace-pre-line">{item.description}</div>
              </div>
            )}

            {item.customerFeedback && (
              <div className="card !p-5">
                <div className="text-sm text-brand-400 mb-2">客户反馈</div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {related.map((r) => (
                <Link
                  key={r.id}
                  href={`/cases/${r.id}`}
                  className="aspect-square rounded-xl overflow-hidden bg-white/5 group relative"
                >
                  {r.coverImage && (
                  ...
                  <div className="absolute bottom-2 left-2 right-2 text-xs line-clamp-2">{r.title}</div>
                </Link>
              ))}
            </div>
          </section>
